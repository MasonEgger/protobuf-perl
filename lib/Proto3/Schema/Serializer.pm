# ABOUTME: Proto3::Schema::Serializer — renders a Schema::File tree as a static
# Perl constructor expression, so AOT-generated modules rebuild it with no parser (§4.12).
use v5.38;
use warnings;

package Proto3::Schema::Serializer;

use Scalar::Util ();

# render_file($file) -> a Perl expression string that, when evaluated,
# reconstructs an equivalent Proto3::Schema::File. Output is deterministic: hash
...
sub render_file ($file) {
    ...
    return _ctor( 'Proto3::Schema::File', \%args );
}
...
sub _message ($message) {
    ...
    return _ctor( 'Proto3::Schema::Message', \%args );
}
...
sub _field ($field) {
    ...
    return _ctor( 'Proto3::Schema::Field', \%args );
}
...
sub _oneof ($oneof) {
    ...
    return _ctor( 'Proto3::Schema::Oneof', \%args );
}
...
sub _enum ($enum) {
    ...
    return _ctor( 'Proto3::Schema::Enum', \%args );
}
...
Proto3::Schema::Serializer - Render a Schema::File as static Perl

=head1 SYNOPSIS

    use Proto3::Schema::Serializer;

    my $expr = Proto3::Schema::Serializer::render_file($file);
    # 'Proto3::Schema::File->new( ... )'

=head1 DESCRIPTION

Produces a Perl source expression that reconstructs a L<Proto3::Schema::File>
(and its full tree of messages, fields, oneofs, enums, and nested types) without
involving the parser. The AOT code generator (L<Proto3::Class::Codegen>) embeds
...
Returns the Perl constructor expression for a L<Proto3::Schema::File>.
