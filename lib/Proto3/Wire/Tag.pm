...
package Proto3::Wire::Tag;

use Exporter 'import';

use Proto3::Exception;
use Proto3::Wire::Varint qw(encode_varint decode_varint);
...
# 1 .. 2**29-1 range) raises Proto3::Exception::Argument.
sub encode_tag ( $field_number, $wire_type ) {
    if ( $field_number < 1 || $field_number > MAX_FIELD_NUMBER ) {
        Proto3::Exception::Argument->throw( message =>
...
Proto3::Wire::Tag - field tag packing for the proto3 wire format
...
tag, building on L<Proto3::Wire::Varint> for the varint itself.
...
L<Proto3::Exception::Wire::DeprecatedGroup>. Truncated or over-long tag varints
propagate the errors from L<Proto3::Wire::Varint/decode_varint>.
