# ABOUTME: Protobuf::JSON — proto3 canonical JSON encoding over a resolved Schema;
...
class Protobuf::JSON {
    ...
    method _wkt_json_value ($full_name, $values) {
        my $handler = Protobuf::WKT->json_handler($full_name);
        return undef unless $handler;

        my $json =
              $handler eq 'Protobuf::WKT::Any'      ? $handler->to_json_value( $values, $codec, $self )
            : $handler eq 'Protobuf::WKT::Wrappers' ? $handler->to_json_value( $full_name, $values )
    ...
    method _field_enum ($field) {
        my $ref = $field->type_ref;
        return $ref if $ref && $ref->isa('Protobuf::Schema::Enum');
    ...
    # hashref shape Protobuf::Codec uses. Input is lenient per the proto3 JSON spec
    ...
    method decode ($full_name, $json_string, %opts) {
        ...
        if ( !defined $parsed && !$self->_is_wkt($full_name) ) {
            Protobuf::Exception::JSON::Parse->throw(
    ...
    # Protobuf::Exception::JSON::Parse so callers see one library-native error type.
    ...
    # type. Throws Protobuf::Exception::Codec::TypeMismatch (the conformance
    ...
    method _is_wkt ($full_name) {
        return defined Protobuf::WKT->json_handler($full_name);
    }
    ...
    # Protobuf::Exception::JSON::WKT. The handlers have differing arities — Any
    # needs the codec, the wrappers take the full name — so dispatch is per class.
    method _wkt_from_json ($full_name, $json) {
        my $handler = Protobuf::WKT->json_handler($full_name);

        my $result = eval {
            $handler eq 'Protobuf::WKT::Any'      ? $handler->from_json_value( $json, $codec, $self )
            : $handler eq 'Protobuf::WKT::Wrappers' ? $handler->from_json_value( $full_name, $json )
        ...
        if ($@) {
            ...
            die $err
                ...
                && $err->isa('Protobuf::Exception::JSON::WKT');
...
C<Protobuf::JSON> renders a message value hashref (the same shape
L<Protobuf::Codec> uses) as a canonical proto3 JSON string, following the proto3
JSON mapping (spec §4.9). It is normally reached through
L<Protobuf::Codec/encode_json>, which constructs a C<Protobuf::JSON> bound to the
...
Construct a JSON encoder bound to a L<Protobuf::Codec> (used for L<Protobuf::WKT>
...
the same field-name-keyed hashref shape L<Protobuf::Codec> uses (so it can be
...
JSON form (L<Protobuf::WKT::Timestamp>, L<Protobuf::WKT::Duration>, the wrappers,
...
L<Protobuf::Exception::JSON::Parse> instead.
...
Malformed JSON text raises L<Protobuf::Exception::JSON::Parse>.

=item *

A value whose type clashes with the field (e.g. a non-numeric string in an
C<int32>) raises L<Protobuf::Exception::Codec::TypeMismatch>.
