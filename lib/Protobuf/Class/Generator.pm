# ABOUTME: Protobuf::Class::Generator — installs a Perl class at runtime from a
# Schema::Message: typed reader/set/clear accessors, construction, descriptor (§4.6).
use v5.38;
use warnings;

package Protobuf::Class::Generator;
...
# Protobuf::Exception::Codec::TypeMismatch. string/bytes accept any non-reference
...
#
# Per-field accessors (accessor base name from Protobuf::Class::Accessor):
...
sub _install_field_accessors ($target, $field, $siblings = undef) {
    my $name = $field->name;
    my $base = Protobuf::Class::Accessor::accessor_name($name);
...
sub _install_which ($target, $oneof_name, $members) {
    my $base = Protobuf::Class::Accessor::accessor_name($oneof_name);
    $members //= [];
    no strict 'refs';    ## no critic (ProhibitNoStrict)
...
# (an explicit clear/unset). Mirrors Protobuf::Codec's type-mismatch contract.
...
=head1 SYNOPSIS
    ...
    Protobuf::Class::Generator->build(
        schema         => $schema,
        message        => $message,             # a Protobuf::Schema::Message
...
a L<Protobuf::Schema::Message>. The generated class is a plain blessed hashref
...
L<Protobuf::Schema::Message> (C<message>) belonging to C<schema>. Returns nothing;
its effect is the newly-populated package symbol table. All three arguments are
required; a missing one raises L<Protobuf::Exception::Argument>. The generated
...
L<Protobuf::Exception::Argument> naming the offending key. Numeric scalar fields
reject a non-number value with L<Protobuf::Exception::Codec::TypeMismatch>.
...
L<Protobuf::Exception::Codec::TypeMismatch> on a mismatch), stores it, and
...
L<Protobuf::Codec>: equivalent to C<< $codec->encode($full_name, $obj->to_hashref) >>
...
possibly-underscored accessor name), suitable for passing to L<Protobuf::Codec>.

=item C<< $class->descriptor >> / C<< $obj->descriptor >>

Returns the L<Protobuf::Schema::Message> the class was built from. Callable as
