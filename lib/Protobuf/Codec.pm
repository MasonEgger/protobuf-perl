# ABOUTME: Protobuf::Codec — high-level encode/decode over a resolved Schema; §4.5.
...
my ( $normalize_int, $encode_signed_varint, $decode_signed_varint ) = do {
    ...
    my $enc = sub ($value) {
        ...
        return Protobuf::Wire::encode_varint($big);
    };
    ...
    # ($value, $rest).
    my $dec = sub ($bytes) {
        my ( $u, $rest ) = Protobuf::Wire::decode_varint($bytes);
...
my %SCALAR_TYPE = do {
    my $W_VARINT = Protobuf::Wire::Tag::WIRE_VARINT();
    my $W_I64    = Protobuf::Wire::Tag::WIRE_I64();
    my $W_LEN    = Protobuf::Wire::Tag::WIRE_LEN();
    my $W_I32    = Protobuf::Wire::Tag::WIRE_I32();

    my $varint  = sub ($v) { Protobuf::Wire::encode_varint($v) };
    ...
    my $zigzag32 = sub ($v) { Protobuf::Wire::encode_zigzag32($v) };
    my $zigzag64 = sub ($v) { Protobuf::Wire::encode_zigzag64($v) };
    my $bool    = sub ($v) { Protobuf::Wire::encode_varint( $v ? 1 : 0 ) };
    my $fixed32 = sub ($v) { Protobuf::Wire::encode_fixed32($v) };
    my $fixed64 = sub ($v) { Protobuf::Wire::encode_fixed64($v) };
    ...
    my $sfixed32 = sub ($v) {
        ...
        $n += 2**32 if $n < 0;
        return Protobuf::Wire::encode_fixed32($n);
    };
    my $sfixed64 = sub ($v) {
        my $big = ( ref $v && $v->isa('Math::BigInt') ) ? $v->copy : Math::BigInt->new("$v");
        $big->badd($TWO_64) if $big->is_neg;
        return Protobuf::Wire::encode_fixed64($big);
    };
    my $float   = sub ($v) { Protobuf::Wire::encode_float($v) };
    my $double  = sub ($v) { Protobuf::Wire::encode_double($v) };
    ...
    my $len     = sub ($v) {
        ...
        return Protobuf::Wire::encode_varint( length $bytes ) . $bytes;
    };
...
class Protobuf::Codec {
    ...
    # bytes/enum/message) raises Protobuf::Exception::Schema here, when the codec
    ...
    method _assert_map_key_type ($map_entry) {
        ...
        Protobuf::Exception::Schema->throw(
            message => sprintf(
    ...
    method encode ($full_name, $values) {
        ...
        if ( !defined $message ) {
            Protobuf::Exception::Codec::UnknownType->throw(
    ...
    method _encode_singular_scalar ($field, $value) {
        ...
        my $tag = Protobuf::Wire::Tag::encode_tag( $field->number, $spec->{wire} );
    ...
    method _encode_repeated ($field, $elements) {
        ...
        if ( $spec && $self->_encode_packed($field) ) {
            ...
            my $tag = Protobuf::Wire::Tag::encode_tag(
                $field->number,
                Protobuf::Wire::Tag::WIRE_LEN(),
            );
            return $tag
                . Protobuf::Wire::encode_varint( length $payload )
                . $payload;
        }
    ...
    method _encode_embedded_message ($field_number, $message_name, $value) {
        ...
        my $tag =
            Protobuf::Wire::Tag::encode_tag( $field_number,
            Protobuf::Wire::Tag::WIRE_LEN() );
        return $tag . Protobuf::Wire::encode_varint( length $bytes ) . $bytes;
    }
    ...
    #
    # Thin adapter: build a Protobuf::JSON bound to this codec and its schema, then
    ...
    # emit_defaults) are forwarded unchanged; see Protobuf::JSON for their meaning.
    method encode_json ($full_name, $values, %opts) {
        require Protobuf::JSON;
        my $json = Protobuf::JSON->new( codec => $self, schema => $schema );
    ...
    # both name and number for enums); see Protobuf::JSON for the full rules and the
    ...
    method decode ($full_name, $bytes) {
        ...
        while ( length $rest ) {
            ...
            ( my $field_number, my $wire_type, $rest ) =
                Protobuf::Wire::Tag::decode_tag($rest);
            ...
            if ( !$field ) {
                my $after =
                    $wire_type == Protobuf::Wire::Tag::WIRE_GROUP_START()
                    ? Protobuf::Wire::skip_group( $rest, $field_number )
                    : Protobuf::Wire::skip_field( $wire_type, $rest );
                if ($preserve_unknown_fields) {
    ...
    # form) is malformed and must fail the parse. Raises a Protobuf::Exception::Wire
    ...
    method _validate_tag ($field_number, $wire_type, $consumed) {
        ...
        my $canonical = length Protobuf::Wire::Varint::encode_varint($tag);
    ...
    method _decode_repeated ($field, $wire_type, $rest, $result) {
        ...
        if (   $spec
            ...
            && $wire_type == Protobuf::Wire::Tag::WIRE_LEN() )
        {
            ( my $block, $rest ) = $self->_read_packed_block($rest);
    ...
    method _read_packed_block ($bytes) {
        my ( $n, $rest ) = Protobuf::Wire::decode_varint($bytes);
        $n = $n->numify if ref $n;
        if ( length($rest) < $n ) {
            Protobuf::Exception::Wire::Truncated->throw(
                message => "expected $n bytes, got " . length($rest),
...
C<Protobuf::Codec> encodes (and, in later steps, decodes) message values against a
resolved L<Protobuf::Schema>. Values are plain Perl hashrefs keyed by field name.
...
Construct a codec bound to a L<Protobuf::Schema>. The schema should already be
resolved (see L<Protobuf::Schema/resolve>) for message-typed fields, though that
...
L<Protobuf::Exception::Schema> immediately, rather than failing later at encode
time.
...
L<Protobuf::JSON>; see that module for the encoding rules, the lenient decoding
...
L<Protobuf::Exception::Wire::DeprecatedGroup>, and truncated input raises
L<Protobuf::Exception::Wire::Truncated>, both surfaced unchanged from the wire
layer.
...
An unknown message type name raises L<Protobuf::Exception::Codec::UnknownType>.

=item *

A value whose type clashes with the field (e.g. a non-numeric string for an
C<int32>) raises L<Protobuf::Exception::Codec::TypeMismatch>, naming the field and
its expected type.

=item *

A schema containing a map field with a disallowed key type raises
L<Protobuf::Exception::Schema> at codec construction (see L</new>).
