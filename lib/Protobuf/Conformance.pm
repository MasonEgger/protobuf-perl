# ABOUTME: Protobuf::Conformance — the conformance testee request handler (spec
...
class Protobuf::Conformance {

    # schema() -> the resolved Protobuf::Schema for the conformance protocol plus
    # the protobuf_test_messages proto3 test message, loaded once from the
    # vendored FileDescriptorSet and cached.
    sub schema ($class) {
        $SCHEMA //= Protobuf::DescriptorSet->load_file( $fds_path->() );
        return $SCHEMA;
    }
...
# A fully-qualified name keeps this callable as Protobuf::Conformance::handle_request.
sub Protobuf::Conformance::handle_request {
    my ($request_bytes) = @_;
    ...
    my $result = Protobuf::Conformance::_process_request($request);
...
sub Protobuf::Conformance::_process_request {
    my ($request) = @_;
    ...
    my ( $values, $error ) =
        Protobuf::Conformance::_parse_payload( $request, $message_type );
    return $error if $error;    # { parse_error => ... } or { skipped => ... }

    # Serialize the parsed message to the requested output format.
    return Protobuf::Conformance::_serialize_payload( $values, $message_type,
        $output_format );
}
...
sub Protobuf::Conformance::_parse_payload {
    my ( $request, $message_type ) = @_;

    if ( exists $request->{protobuf_payload} ) {
        my $values = eval {
            Protobuf::Conformance->codec->decode( $message_type,
    ...
    if ( exists $request->{json_payload} ) {
        ...
        my $values = eval {
            Protobuf::Conformance->json->decode(
...
# serialize_error.
sub Protobuf::Conformance::_serialize_payload {
    my ( $values, $message_type, $output_format ) = @_;

    if ( $output_format == $WIRE_PROTOBUF ) {
        my $bytes = eval {
            Protobuf::Conformance->codec->encode( $message_type, $values );
        };
    ...
    my $string = eval {
        Protobuf::Conformance->json->encode( $message_type, $values );
    };
    return { serialize_error => "$@" } if $@;
...
sub Protobuf::Conformance::parse_runner_output {
    my ($text) = @_;
    $text //= '';
...
# no runner installed).
sub Protobuf::Conformance::find_runner {
...
sub Protobuf::Conformance::run_stdio {
    my ( $in_fh, $out_fh ) = @_;
    binmode $in_fh;
    binmode $out_fh;
    ...
    my $served = 0;
    while ( defined( my $request_bytes = Protobuf::Conformance::_read_frame($in_fh) ) ) {
        my $response_bytes = Protobuf::Conformance::handle_request($request_bytes);
        Protobuf::Conformance::_write_frame( $out_fh, $response_bytes );
        $served++;
    }
    return $served;
}
...
sub Protobuf::Conformance::_read_frame {
    my ($fh) = @_;

    my $len_bytes = Protobuf::Conformance::_read_exact( $fh, 4 );
    return undef unless defined $len_bytes;    # clean EOF before next frame

    my $length = unpack 'V', $len_bytes;
    my $payload = Protobuf::Conformance::_read_exact( $fh, $length );
...
sub Protobuf::Conformance::_read_exact {
    my ( $fh, $want ) = @_;
    return '' if $want == 0;
...
sub Protobuf::Conformance::_write_frame {
    my ( $fh, $bytes ) = @_;
...
=head1 SYNOPSIS
    ...
    #   $ protobuf-conformance < requests > responses

=head1 DESCRIPTION

C<Protobuf::Conformance> implements the testee side of Google's protobuf
...
L<Protobuf::DescriptorSet> — so the testee needs no C<protoc> at runtime. All
logic lives in this module; C<bin/protobuf-conformance> is a thin stdin/stdout
...
=head2 parse_runner_output
    ...
    my $verdict = Protobuf::Conformance::parse_runner_output($runner_stdout);
...
=head2 find_runner
    ...
    my $path = Protobuf::Conformance::find_runner;
...
=head2 run_stdio
    ...
    my $count = Protobuf::Conformance::run_stdio(\*STDIN, \*STDOUT);
...
=head2 schema / codec / json
    ...
    my $schema = Protobuf::Conformance->schema;
    my $codec  = Protobuf::Conformance->codec;
    my $json   = Protobuf::Conformance->json;

Lazily-built, cached singletons: the resolved L<Protobuf::Schema> for the
conformance protocol (from C<share/proto/conformance.fds>), and the
L<Protobuf::Codec> / L<Protobuf::JSON> handlers built over it.
